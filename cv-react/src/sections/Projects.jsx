  ...
  return (
    <section id="projects" className="pt-24 pb-16 px-4 sm:px-8 lg:px-20">
      <div className="max-w-6xl mx-auto mb-10">
        <h2 className="text-[1.75rem] md:text-[2.3125rem] font-extrabold text-gray-900 leading-tight flex items-center gap-3
          before:content-[''] before:block before:w-[3px] before:h-[1.625rem] md:before:h-[2.125rem] before:bg-blue-500 before:rotate-[15deg] before:rounded-sm before:shrink-0
        ">
          Proyectos Destacados
        </h2>
        <p className="mt-4 text-[0.9375rem] md:text-[1.0625rem] text-gray-500 max-w-2xl leading-[1.375rem]">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-5">
        {projects.map((proj, index) => (
          <motion.div
            ...
            viewport={{ once: true }}
            whileHover={{ y: -3 }}
            className="bg-blue-50/50 border border-blue-100 rounded-xl p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)] transition-shadow hover:shadow-[0_4px_12px_rgba(0,0,0,0.06)]"
          >
            <div className="w-full h-36 bg-gradient-to-br from-blue-100 to-blue-200 rounded-lg mb-5 flex items-center justify-center">
            ...
            <p className="text-sm text-gray-500 leading-[1.375rem] mb-4">{proj.desc}</p>
            <div className="flex flex-wrap gap-2">
              {proj.tech.map(tech => (
                <span key={tech} className="px-2.5 py-1 bg-white border border-blue-100 rounded-full text-[0.6875rem] text-blue-600 font-medium">
                  {tech}
                </span>
              ))}
            </div>
